...
GLOBAL_GRAYSCALE_MODE = False  # 전역 변수로 이미지 모드 저장 (True: 흑백, False: 컬러, None: 미결정)
SCAN_INTERVAL = 5  # 폴더 스캔 간격 (초)
PROCESSED_FILES_PREFIX = "processed_files_" # 처리된 파일 목록 파일명 접두사
PROCESSED_FILE_DELIMITER = "," # 처리된 파일 목록 파일 구분자
        ...
        with open(filepath, 'r') as f:
            for line in f:
                ...
                if len(parts) == 2:
                    ...
                    processed[file_path] = float(timestamp) # 수정 시간을 float으로 저장
    return processed
