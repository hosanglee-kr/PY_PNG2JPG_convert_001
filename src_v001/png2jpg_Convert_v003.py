...
def get_processed_files_filename(output_base_folder, base_folder_name, date_str=None):
    ...
    year_month = date_str[:6]  #<ctrl3348>MM 형식 추출
    ...
    while True:
        ...
        for root, folders, files in os.walk(watch_folder):
            ...
            for folder in folders:
                if folder in ['NG', 'OK', 'NG_OK']: # NG, OK, NG_OK 폴더만 확인
                #if os.path.basename(folder) in ['NG', 'OK', 'NG_OK']: # NG, OK, NG_OK 폴더만 확인
                    ...
                    #for sub_root, _, sub_files in os.walk(root):
                        for folder_side in folder_yearMonth_sides:
                            ...
                            #if os.path.basename(sub_root) in ['LEFT', 'LINE', 'LINE_TAP', 'LOAD', 'LOAD_TAP', 'RIGHT', 'TOP']:
        ...
        for root, _, files in os.walk(watch_folder):
            # NG, OK, NG_OK 폴더만 확인
            if os.path.basename(root) in ['NG', 'OK', 'NG_OK']:
                for sub_root, _, sub_files in os.walk(root):
                    # 특정 하위 폴더만 처리하도록 필터링
                    if os.path.basename(sub_root) in ['LEFT', 'LINE', 'LINE_TAP', 'LOAD', 'LOAD_TAP', 'RIGHT', 'TOP']:
                        for filename in sub_files:
                            if filename.lower().endswith(".png"):
                                png_path = os.path.join(sub_root, filename)
